...
class CveRepository(ABC):
    @abstractmethod
    def upsert_raw(self, entry: CveRecord, *, page: int) -> None:
        raise NotImplementedError

    @abstractmethod
    def get_raw(self, cve_id: str) -> Optional[CveRecord]:
        raise NotImplementedError
...
class FileCveRepository(CveRepository):
    ...
    def get_raw(self, cve_id: str) -> Optional[CveRecord]:
        ...
        if not path.exists():
            return None
        try:
            return CveRecord.model_validate_json(path.read_text(encoding="utf-8"))
    ...
    def query_raw(
        ...
        *,
        ...
        for raw_path in sorted(self.raw_dir.glob("CVE-*.json")):
            try:
                item = CveRecord.model_validate_json(
...
class SqliteCveRepository(CveRepository):
    ...
    def _init_github_sbom_tables(self, conn: sqlite3.Connection) -> None:
        ...
        but bootstrapped here so any SqliteCveRepository connection guarantees they exist.
        """
        conn.execute(
            """
    ...
    def get_raw(self, cve_id: str) -> Optional[CveRecord]:
        ...
        if row is None:
            return None
        try:
            return CveRecord.model_validate_json(row["payload"])
        except Exception:
            return None
    ...
    def query_raw(
        ...
        *,
        ...
        for row in rows:
            try:
                items.append(CveRecord.model_validate_json(row["payload"]))
            except Exception:
                continue
        return CveQueryResult(
...
class DualWriteCveRepository(CveRepository):
    def __init__(self, primary: CveRepository, secondary: CveRepository) -> None:
